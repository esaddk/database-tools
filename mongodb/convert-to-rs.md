...
1. **Clone the Repository:**
   ...
   ```bash
   git clone https://github.com/your-username/mongodb-replica-set-script.git
   ```

2. **Navigate to the Script Directory:**

   ```bash
   cd mongodb-replica-set-script
   ```

3. **Update Script Variables:**

   Open the script (`mongodb_replica_set_init.sh`) in a text editor and update the following variables:
...
4. **Run the Script:**
   ...
   ```bash
   chmod +x mongodb_replica_set_init.sh
   ./mongodb_replica_set_init.sh
   ```
...
5. **Review Logs:**
   ...
   ```bash
   cat mongodb_replica_set_init.log
   ```
